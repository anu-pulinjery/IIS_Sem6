...
class blockChain:
    ...
    def process_transactions(self,txns,miners,com,bhsa,reward):
        a=[]
        txns.sort(key=lambda txn: (-txn[3], txn[1]))
        for i in range(0,len(txns)//self.block_limit+1,1):
            a=[]
            ...
            for i in range(4):
                while j in range(len(txns)):
                    ...
                    if sender in self.accnts and self.accnts[sender] >= amount and receiver in self.accnts:
                        ...
                        a[i]=[sender, receiver, amount, incentive]
                        txns.remove(a[i])
                        break
                    j=j+1     
            batch=a[0:self.block_limit]
            for k in range(len(batch)):
                if batch[k]==[]:
                    batch=batch[0:k]
                    break
                batch[k]=transaction(batch[k][0],batch[k][1],batch[k][2],batch[k][3])
            m=0
            miner_id=-1
            val=0
...
for _ in range(num_miners):
    j=input().split()
    miners.append(j[0])
    if j[0] not in accounts_list:
        blockChain.add_account(j[0],0)
    com.append(int(j[1]))
    a=[]
    for i in range(7):
        a.append(int(j[2+i]))
...
blockchain.process_transactions(transactions,miners,com,bhsa,reward)
